...
io.on('connection', function(socket){
  ...
  socket.on('getUsers', function(lobby){
    Lobby.findOne({lobby: lobby}, function(err, doc){
      ...
      doc["count"] = clients.server.eio.clientsCount;
...
io.on('connection', function (socket){
  socket.on('postGameCreate', function(user, room ){
    console.log(user, room);
    socket.room = room + "post";
    socket.join(socket.room);
    io.in(socket.room).emit('postGameUsers', user);
  })
  // socket.on('postChat', function(){
  //   console.log(user, room);
  //   io.in(socket.room).emit('postGameUsers', user);
  //   // io.in(soc)
  // })
...
io.on('connection', function (socket) {
  ...
  socket.on('createRoom', function(room){
    socket.room = room;
    socket.join(socket.room);
    console.log("game socket : ",socket.room);
  ...
  socket.on('updateTickerScore', function(data){
    ...
    io.in(socket.room + 'post').emit('domTickerScore', data);
  })
